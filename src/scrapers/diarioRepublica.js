...
export async function scrapeDiarioRepublica() {
  console.log('🔍 Scraping Diário da República...');
  
  try {
    const url = 'https://diariodarepublica.pt/dr/home';
    ...
    $('.entry, .document-item, article, .list-item, .item').each((index, element) => {
      ...
      if (titulo && link && titulo.length > 10) {
        const urlCompleto = link.startsWith('http') ? link : `https://diariodarepublica.pt${link}`;
        
        novosDocumentos.push({
          titulo,
          url: urlCompleto,
          resumo: resumo || titulo.substring(0, 200),
          fonte: 'diario_republica'
        });
      }
    });
    
    console.log(`📄 Encontrados ${novosDocumentos.length} documentos no Diário da República`);
    ...
    console.log(`✅ Diário da República: ${novosGuardados} novos documentos guardados`);
    return novosGuardados;
    
  } catch (error) {
    console.error('❌ Erro no scraping Diário República:', error.message);
    return 0;
  }
}